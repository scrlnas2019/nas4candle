...
def add_subparser(subparsers):
    ...
    parser_parse = subparsers.add_parser(subparser_name, help='Tool to parse "nas4candle.nasapi.log" and produce a JSON file.')
    parser_parse.add_argument('path', type=str, help=
            f'The parsing script takes only 1 argument: the relative path to the log file. If you want to compute the workload data with \'balsam\' you should specify a path starting at least from the workload parent directory, eg. \'nas_exp1/nas_exp1_ue28s2k0/nas4candle.nasapi.log\' where \'nas_exp1\' is the workload.')

    return subparser_name, function_to_call

def main(path, *args, **kwargs):
    print(f'Path to nas4candle.nasapi.log file: {path}')
